	...
	*/
	...
	case "frontpage.html":
		...
		if ( $_POST["type"] == "photoComment" && (int)$_POST["statusId"] > 0  && strlen($_POST["comment"]) > 0 && $_SESSION["demo"] != TRUE  )
		{
				...
				$record["statusMessage"] = "Kommenterade foto: <a href=\"http://dev.flator.se/media/photos/".(int)$_POST["photoId"].".html\">".addslashes($_POST["comment"])."</a>";
				...
				if (strlen($_POST["comment"])> 25) $_POST["comment"] = substr($_POST["comment"],0,25)." ...";
				$record["statusMessage"] = "<a href=\"http://dev.flator.se/user/".$userProfile["username"].".html\">".$userProfile["username"]."</a> kommenterade ett av dina fotografier: <a href=\"http://dev.flator.se/media/photos/".(int)$_POST["photoId"].".html\">".addslashes($_POST["comment"])."</a>";
				$record["mostRecent"] = "NO";
				$record["statusType"] = "photoComment";
				$record["private"] = "YES";
				$DB->AutoExecute( "fl_status", $record, 'INSERT');
		...
		if ($editBlog == TRUE && $_SESSION["demo"] != TRUE ) {
			...
			if ( (int)$_SESSION["rights"] > 1 )
			{
				...
				if ( $_POST["type"] == "writeBlog" && strlen($_POST["content"]) > 0 )
				{
						...
						$record["insDate"] = date("Y-m-d H:i:s");
						$record["userid"] = (int)$userProfile["id"];
						$record["statusMessage"] = "Nytt blogginlägg: <a href=\"http://dev.flator.se/blogs/".$userProfile["username"].".html\">".addslashes($_POST["subject"])."</a>";
						$record["mostRecent"] = "NO";
		...
		if (strlen($currentAlbum["id"]) > 0) {
			...
			if ($currentAlbum["userId"] != $userProfile["id"]) {
				...
				if ( $_POST["type"] == "reportPhoto"  && $_SESSION["demo"] != TRUE )
				{
						...
						$DB->AutoExecute( "fl_reports", $record, 'INSERT' ); 
						$tmpMessage = "Rapport för fotografi ID: <a href=\"http://dev.flator.se/media/photos/".(int)$_POST["photoId"].".html\">".(int)$_POST["photoId"]."</a> med anledning:\n".$_POST["comment"];
