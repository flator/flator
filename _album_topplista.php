<?php
$metaTitle = "Flator.se - Topplistor - Album";
$numPerPage = 50;
...
else
{
	...
	$currentLink = array();
		...
		$q = "SELECT fl_albums.*, UNIX_TIMESTAMP(fl_albums.modifiedDate) AS unixTime, fl_users.username, fl_users.insDate AS joinedDate FROM fl_albums LEFT JOIN fl_users ON fl_users.id = fl_albums.userId ORDER BY fl_albums.noViews DESC LIMIT 30";
		$albums = $DB->CacheGetAssoc( 3600*48, $q, FALSE, TRUE );
...
<div id=\"divHeadSpace\" style=\"border: 0px; line-height: 10px\">&nbsp;</div><div style=\"float: left; \"><div style=\"padding-top: 6px; padding-bottom: 4px;border:none; border-bottom: 1px dotted #c8c8c8; width:592px; margin-bottom:10px;\" width=\"595px\"><h3>De 30 mest sedda albumen</h3></div></div>";
