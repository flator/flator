<?php
$metaTitle = "Flator.se - Senaste - Album";
$numPerPage = 50;
...
else
{
	...
	$currentLink = array();
		...
		$q = "SELECT fl_albums.*, UNIX_TIMESTAMP(fl_albums.modifiedDate) AS unixTime, fl_users.username, fl_users.insDate AS joinedDate FROM fl_albums LEFT JOIN fl_users ON fl_users.id = fl_albums.userId ORDER BY fl_albums.modifiedDate DESC LIMIT 50";
	...
	if ( count( $albums ) > 0 )
	{
		while ( list( $key, $value ) = each( $albums ) )
		{
			...
			$albumImage = $DB->CacheGetRow( 20*60, $q, FALSE, TRUE );
...
<div id=\"divHeadSpace\" style=\"border: 0px; line-height: 10px\">&nbsp;</div><div style=\"float: left; \"><div style=\"padding-top: 6px; padding-bottom: 4px;border:none; border-bottom: 1px dotted #c8c8c8; width:592px; margin-bottom:10px;\" width=\"595px\"><h3>De 50 senast uppdaterade albumen</h3></div></div>";
...
<table border=\"0\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"font-size: 12px;\">";
	...
	if ( count( $albums ) > 0 )
	{
 	...
 	<td style=\"width: 140px;padding-bottom: 10px\" valign=\"top\"><a href=\"" . $baseUrl . "/media/album/" . $key . ".html\">" . (stripslashes($albums[ $key ]["name"]) == "" ? 'Namnlös' : stripslashes($albums[ $key ]["name"])) . "</a><br />" . $onlineTime . "
