...
$q = "SELECT *, UNIX_TIMESTAMP(startDate) AS startTime, UNIX_TIMESTAMP(endDate) AS endTime FROM fl_events WHERE (eventType = 'Event' OR eventType = '') AND public = 'YES' AND deleted = 'NO' AND UNIX_TIMESTAMP( endDate ) >= UNIX_TIMESTAMP( NOW() ) ORDER BY startDate ASC";
