<?php
$metaTitle = "Flator.se - Topplistor - Nya filmer";
$numPerPage = 50;
...
if ( (int)$_SESSION["rights"] < 2 )
{
	$publicMenu = TRUE;
	$memberMenu = FALSE;
	$loginUrl = $baseUrl . "/nya_videos.html";
...
else
{
	...
	$currentLink = array();
		...
		$q = "SELECT fl_images.*, UNIX_TIMESTAMP(fl_images.insDate) AS unixTime, fl_users.username FROM fl_images LEFT JOIN fl_users ON fl_users.id = fl_images.userId WHERE imageType = 'albumVideo' ORDER BY unixTime DESC LIMIT 30";
...
<div id=\"divHeadSpace\" style=\"border: 0px; line-height: 10px\">&nbsp;</div><div style=\"float: left; \"><div style=\"padding-top: 6px; padding-bottom: 4px;border:none; border-bottom: 1px dotted #c8c8c8; width:592px; margin-bottom:10px;\" width=\"595px\"><h3>De 30 senaste filmerna</h3></div></div>";
