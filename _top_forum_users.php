<?php
$metaTitle = "Flator.se - Topplistor - Forum - Användare";
$numPerPage = 50;
...
if ( (int)$_SESSION["rights"] < 2 )
{
	$publicMenu = TRUE;
	$memberMenu = FALSE;
	$loginUrl = $baseUrl . "/top_forum_users.html";
...
else
{
	...
	$currentLink = array();
		...
		$q = "SELECT fl_users.* FROM fl_users WHERE fl_users.rights > 1 ORDER BY forumPostsLastWeek DESC LIMIT 0, 20";
...
#	}
	...
	$searchResult = $DB->CacheGetAssoc( 3*60, $q, FALSE, TRUE );
...
<div id=\"divHeadSpace\" style=\"border: 0px; line-height: 10px\">&nbsp;</div><div style=\"float: left; \"><div style=\"padding-top: 6px; padding-bottom: 4px;border:none; border-bottom: 1px dotted #c8c8c8; width:592px; margin-bottom:10px;\" width=\"595px\"><h3>De 20 mest aktiva medlemmarna på forumet den senaste veckan</h3></div></div>";
...
$printed = 0;
	if ( count( $searchResult ) > 0 )
	{
		...
		while ( list( $key, $value ) = each( $searchResult ) )
		{
			...
			$onlineRow = $DB->CacheGetRow( 60, $q, FALSE, TRUE );
...
$body.= "<tr>
 	...
 	<td style=\"padding-bottom: 30px;\" valign=\"top\"><a href=\"" . $baseUrl . "/user/" . $searchResult[ $key ]["username"] . ".html\">" . $searchResult[ $key ]["username"] . "</a> " . stripslashes($searchResult[ $key ]["status"]) . "<br /><span class=\"email_date\">Inlägg skrivna på forumet den senaste veckan: <b>" . $searchResult[ $key ]["forumPostsLastWeek"] . "</b></span>";
