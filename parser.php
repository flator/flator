...
||   are available as of Recruiting Parser 1.1 .
...
class parser {
	...
	private $emoticons = array('confident', 'happy', 'crying', 'friendly', 'evil', 'panic', 'indifferent', 'angel', 'teasing', 'angry', 'polite', 'mad', 'tearing', 'yelling', 'sad', 'sceptical', 'confident');
	private $altEmoticons = array(':)', ':D', ';(', ';)', '&gt;:)', ':O', ':|', 'O:)', ':P', ':(', ':3', ':X', ':@', ':()', ':/', ':\\', ':-)');
	
	protected $rpVersion = '1.0';
...
public function smilies($whichOne) {
	...
	$this->emoticons[16] => '');
...
// ARGUMENTS :
	...
	# $showFormattedTwice = shows formatted text once more, but in special look
			# then you can also use a constant RP_BUFFER through documents to access that text
			
	# $onShutdown = put here some PHP code (with or without newlines) and it will be executed when script finishes with its work
	...
	// P a r s e
	$s = preg_replace("#\[url\=(.*)\](.*)\[\/url\]#Ui", "<a rel=\"nofollow\" href=\"$1\" target=\"_blank\">$2</a>", $s);
	$s = preg_replace("#\[url\](.*)\[\/url\]#Ui", "<a rel=\"nofollow\" href=\"$1\" target=\"_blank\">$1</a>", $s);
...
public function deparse($string, $removeBr = 1, $return = 1) {
	...
	"[url=$1]$2[/url]" => "\<a rel=\"nofollow\" href\=\"(.*)\" target\=\"_blank\"\>(.*)\<\/a\>",
