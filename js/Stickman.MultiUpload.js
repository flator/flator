...
var MultiUpload = new Class(
{
	...
	addRow:function(){
		if( this.max == 0 || this.elements.length <= this.max ){
			...
			var delete_button = new Element(
				'img',
				{
					'src':'http://www.flator.se/img/cross_small.gif',
					'alt':'Delete',
					'title':'Delete',
					'events':{
						'click':function( uid ){
		...
		if( confirm( 'Vill du ta bort\r\n' +  deleted_row.element.value + '\r\nfrån listan över bilder att ladda upp?' ) ){
			...
			for( var i = 0; i < this.elements.length; i++ ){
				if( $defined( this.elements[ i ] ) ){
					this.elements[ i ].element.name = 'bild[]';
					this.elements[ i ].element.style.border = 'solid 1px #aa567a';
	...
	initializeElement:function( element ){
		...
		// Set the name
		element.name = 'bild[]';
		element.style.border = 'solid 1px #aa567a';
