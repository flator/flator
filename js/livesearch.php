	...
	else
	{
		if ( mysql_num_rows( $result ) > 0 )
		{
			...
			while( $suggest = mysql_fetch_array( $result ) )
			{
				...
				if ( $i == 10 )
				{
					echo "<div style=\"color:#A09C96; font-family:verdana; font-size:9px; font-weight:bold; margin-left:4px; margin-top:4px; margin-bottom:4px;\">Maximalt 9 visas h&auml;r, klicka p&aring; f&ouml;rstoringsglaset eller tryck 'enter' f&ouml;r att visa fler.</div>";
				}
				else
				{
					...
					if ( count( $guestImage ) > 0 )
					{
						...
						$guestImage["imageUrl"] = "http://www.flator.se/user-photos/".str_replace($usedImagesServerPaths, "", $guestImage["serverLocation"])."/profile-small/";
					...
					echo "<div id=\"rad".$i."\" style=\"clear:both;margin-left:0px;padding-left:4px;padding-top:3px; padding-bottom:5px;border-bottom:1px dotted #c8c8c8;cursor:hand;cursor:pointer;min-height:40px;\" onMouseOver=\"document.getElementById('rad".$i."').style.background='#a35e7b';document.getElementById('rubrik".$i."').style.color='#ffffff';document.getElementById('text".$i."').style.color='#ffffff';\" onMouseOut=\"document.getElementById('rad".$i."').style.background='#ffffff';document.getElementById('rubrik".$i."').style.color='';document.getElementById('text".$i."').style.color='#A09C96';\" onClick=\"javascript: location.href='".$baseUrl."/user/" . $suggest["username"] . ".html/';\" align=\"top\">";
					echo "<span style=\"font-size: 11px\"><a id=\"rubrik".$i."\" href=\"".$baseUrl."/user/" . $suggest["username"] . ".html\" style=\"text-decoration: none;\">" . $avatar . "";
					echo " ";
#if ( $user_id == 1 ) echo $q;
					echo eregi_replace( $search, "<b>" . $search . "</b>", $suggest["username"] )."</a>";


									$statusMessage = truncate(stripslashes($statusRow["statusMessage"]), 37, "..", TRUE, TRUE);
					...
					echo "".htmlentities($statusMessage, ENT_COMPAT, "ISO-8859-1",FALSE)."</span>";
						...
						echo "</span><div style=\"clear:both;\"></div></div>\n";
		...
		{
			echo "Inga matchande medlemmar funna.\n";
		}
	}
	echo "";
}
