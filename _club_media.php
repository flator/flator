<?php
$metaTitle = "Flator.se - Festbilder - Klubbalbum";
$numPerPage = 50;
...
if ( (int)$_SESSION["rights"] < 2 )
{
	$publicMenu = TRUE;
	$memberMenu = FALSE;
	$loginUrl = $baseUrl . "/club_media.html";
...
else
{
	...
	if ( $_POST )
	{
		if ( $_POST["type"] == "addAlbum"  && $_SESSION["demo"] != TRUE )
		{
		...
		if ( $_POST["type"] == "editAlbum"  && $_SESSION["demo"] != TRUE )
		{
		...
		if ( $_POST["type"] == "uploadPhoto"  && $_SESSION["demo"] != TRUE )
		{
	...
	if ( $_GET["sortBy"] == "asc" )
	{
		$q = "SELECT fl_albums.*, UNIX_TIMESTAMP(fl_albums.modifiedDate) AS unixTime, fl_users.username, fl_users.insDate AS joinedDate FROM fl_albums LEFT JOIN fl_users ON fl_users.id = fl_albums.userId WHERE fl_albums.clubAlbum = 'YES' ORDER BY fl_albums.insDate ASC";
		$albums = $DB->GetAssoc( $q, FALSE, TRUE );
		$currentLink["asc"] = " class=\"current\"";
	}
	elseif ( !$_GET["sortBy"] )
	{
		$q = "SELECT fl_albums.*, UNIX_TIMESTAMP(fl_albums.modifiedDate) AS unixTime, fl_users.username, fl_users.insDate AS joinedDate FROM fl_albums LEFT JOIN fl_users ON fl_users.id = fl_albums.userId WHERE fl_albums.clubAlbum = 'YES' ORDER BY fl_albums.insDate DESC";
		$albums = $DB->GetAssoc( $q, FALSE, TRUE );
		$currentLink["desc"] = " class=\"current\"";
	}
	elseif ( $_GET["sortBy"] == "visits" )
	{
		$q = "SELECT fl_albums.*, UNIX_TIMESTAMP(fl_albums.modifiedDate) AS unixTime, fl_users.username, fl_users.insDate AS joinedDate FROM fl_albums LEFT JOIN fl_users ON fl_users.id = fl_albums.userId WHERE fl_albums.clubAlbum = 'YES' ORDER BY fl_albums.noViews DESC";
		$albums = $DB->GetAssoc( $q, FALSE, TRUE );
...
$body .= "<span onMouseOver=\"document.albumEdit" . $key . ".src='" . $baseUrl . "/img/symbols/gif_red/redigera.gif'\" onMouseOut=\"document.albumEdit" . $key . ".src='" . $baseUrl . "/img/symbols/gif_purple/redigera.gif'\"><a  href=\"#noexist\" onClick=\"showPopup('popupEditAlbum" . $albums[ $key ]["id"] . "');\" style=\"font-weight: normal\"><img src=\"" . $baseUrl . "/img/symbols/gif_purple/redigera.gif\" name=\"albumEdit" . $key . "\" align=\"ABSMIDDLE\" border=\"0\" />Redigera</a></span>";
  ...
  $body .= "</p>
	...
	$body.= "<div class=\"underlinks\"><a href=\"?do=stopClub&albumId=".$key."\">Inaktivera klubbalbum</a></div>";
		} else {
	$body.= "<div class=\"underlinks\"><a href=\"?do=setClub&albumId=".$key."\">Aktivera klubbalbum</a></div>";
		}
