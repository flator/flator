...
if ( $_POST )
{
	...
	if ( strlen( $_POST["firstName"] ) < 1 ) $message.= "<li>Du m&aring;ste ange ett <b>f&ouml;rnamn</b>.</li>";
	if ( strlen( $_POST["lastName"] ) < 1 ) $message.= "<li>Du m&aring;ste ange ett <b>efternamn</b>.</li>";
	...
	if ( strlen( $message ) < 1 )
	{
		...
		$record["personalCodeNumber"] = $_POST["personalCodeNumber"];
		...
		else
		{
			...
			$thankyou = "<li>Du &auml;r <b>nu</b> en registrerad medlem p&aring; Flator.se! <a href=\"http://www.flator.se/\">Klicka h&auml;r</a> f&ouml;r att komma till hemsidan och logga in.</li>";
			$thankyou.= "<li>Ett mail har skickats till din e-postadress d&auml;r du ombeds bekr&auml;fta adressen. Innan du gjort detta kommer andra medlemmar inte att kunna se din profil och du kommer inte kunna skicka meddelanden.</li>";
		}
		if ( $userId > 0 )
		{
		...
		else
		{
			...
			$DB->AutoExecute( "fl_users", $record, 'INSERT' ); 			
			$userId = $DB->Insert_ID();
		}
		...
		if ( $_POST["email"] != $email )
		{
			...
			if ( count( $row ) > 0 )
			{
				...
				if ( strlen( $subject ) < 1 )
				{
					$subject = "Flator.se - Bekr&auml;fta e-postadress";
				}
				$tmpMessage = $message;
		...
		if ( count( $row ) > 0 )
		{
			...
			if ( strlen( $subject ) < 1 )
			{
				$subject = "Flator.se - V&auml;lkommen!";
			}
			$tmpMessage = $message;
...
<p>Du som f&aring;tt en inbjudan per mail till att bli medlem p&aring; Flator.se kan h&auml;r registrera dig. N&auml;r du skapat kontot kan du logga in direkt.</p>

<p style=\"margin-bottom: 40px; font-weight: bold; color: rgb(199,70,61);\">En inbjudningskod kan endast anv&auml;ndas en g&aring;ng!</p>";
...
<p style=\"margin-bottom: 35px;\"><label for=\"invitationCode\">Inbjudningskod:</label> <input type=\"text\" id=\"invitationCode\" name=\"invitationCode\" value=\"" . $invitationCode . "\" /> (17 tecken)</p>

<p><label for=\"username\">Anv&auml;ndarnamn:</label> <input type=\"text\" id=\"username\" name=\"username\" value=\"" . $_POST["username"] . "\" /></p>
...
<p style=\"margin-bottom: 35px;\"><label for=\"personalCodeNumber\">Personnummer:</label> <input type=\"text\" id=\"personalCodeNumber\" name=\"personalCodeNumber\" value=\"" . $_POST["personalCodeNumber"] . "\" /> F&ouml;r att bekr&auml;fta att du &auml;r tjej beh&ouml;ver vi ditt personnummer.</p>
