...
if ( $_POST["type"] == "uploadPhoto2"  && $_SESSION["demo"] != TRUE )
	...
	if ( $_GET["sortBy"] == "asc" )
	{
		$q = "SELECT fl_images.*, UNIX_TIMESTAMP(fl_images.insDate) AS unixTime, fl_users.username, fl_users.insDate AS joinedDate FROM fl_images LEFT JOIN fl_users ON fl_users.id = fl_images.userId WHERE fl_images.albumId = " . (int)$currentAlbum["id"] . " ORDER BY fl_images.insDate ASC";
		$albumPhotos_pre = $DB->GetAssoc( $q, FALSE, TRUE );
		$currentLink["asc"] = " class=\"current\"";
	}
	elseif ( !$_GET["sortBy"] )
	{
		$q = "SELECT fl_images.*, UNIX_TIMESTAMP(fl_images.insDate) AS unixTime, fl_users.username, fl_users.insDate AS joinedDate FROM fl_images LEFT JOIN fl_users ON fl_users.id = fl_images.userId WHERE fl_images.albumId = " . (int)$currentAlbum["id"] . " ORDER BY fl_images.insDate DESC";
		$albumPhotos_pre = $DB->GetAssoc( $q, FALSE, TRUE );
		$currentLink["desc"] = " class=\"current\"";
	}
	elseif ( $_GET["sortBy"] == "visits" )
	{
		$q = "SELECT fl_images.*, UNIX_TIMESTAMP(fl_images.insDate) AS unixTime, fl_users.username, fl_users.insDate AS joinedDate FROM fl_images LEFT JOIN fl_users ON fl_users.id = fl_images.userId WHERE fl_images.albumId = " . (int)$currentAlbum["id"] . " ORDER BY fl_images.noViews DESC";
		$albumPhotos_pre = $DB->GetAssoc( $q, FALSE, TRUE );
...
$i = 1;
	...
	$albums[$currentAlbum["id"]] = $currentAlbum["name"];
...
*/
	...
	<script type="text/javascript">
		...
		s1.addVariable(\'enablejs\',\'true\');
          ...
          s1.addVariable(\'frontcolor\',\'0xCC00CC\');
          s1.addVariable(\'lightcolor\',\'0x660066\');
...
if ($currentPhoto["number"] > 1) {
	$prevUrl = $baseUrl . "/media/photos/".$albumPhotos[($currentPhoto["number"]-1)]["id"].".html";
...
if ($currentPhoto["number"] < count($albumPhotos)) {
	$nextUrl = $baseUrl . "/media/photos/".$albumPhotos[($currentPhoto["number"]+1)]["id"].".html";
...
if ( $editMedia == TRUE )
{
	$body.= rightMenu('album');
}
else
{
	$body.= rightMenu('userAlbum');
}
$body.= "</div>";
