...
#	}
	...
	if ( count( $profileImage ) > 0 )
	{
		$avatar = "<img src=\"" . $baseUrl . "/user-photos/" . str_replace('http://www.flator.se/rwdx/user/', '', $profileImage["imageUrl"]) . "/profile/\" />";
	}
	else
	{
...
window.open('http://www.flator.se/videochatt/','videochat','width=800,height=620');
</script>
";
		}
...
if ($userStatus[ $key ]["statusType"] == "forumEntry") {
			...
			$symbol = '<img src="http://www.flator.se/img/symbols/gif_purple/bild.gif" style="vertical-align:top;margin-top:3px;" border="0">';
			} elseif ($userStatus[ $key ]["statusType"] == "personalMessage") {
			$symbol = '<img src="http://www.flator.se/img/symbols/gif_purple/logga_in.gif" style="vertical-align:top;margin-top:3px;" border="0">';
			} elseif ($userStatus[ $key ]["statusType"] == "newFriend") {
			$symbol = '<img src="http://www.flator.se/img/symbols/gif_purple/van.gif" style="vertical-align:top;margin-top:3px;" border="0">';
			} elseif ($userStatus[ $key ]["statusType"] == "blogEntry") {
			$symbol = '<img src="http://www.flator.se/img/symbols/gif_purple/blogg.gif" style="vertical-align:top;margin-top:3px;" border="0">';
			} elseif ($userStatus[ $key ]["statusType"] == "blogComment") {
			$symbol = '<img src="http://www.flator.se/img/symbols/gif_purple/lamna_kommentar.gif" style="vertical-align:top;margin-top:3px;" border="0">';
			} elseif ($userStatus[ $key ]["statusType"] == "addedEvent") {
			$symbol = '<img src="http://www.flator.se/img/symbols/gif_purple/typ_av_event.gif" style="vertical-align:top;margin-top:3px;" border="0">';
			} elseif ($userStatus[ $key ]["statusType"] == "photoComment") {
			$symbol = '<img src="http://www.flator.se/img/symbols/gif_purple/bild.gif" style="vertical-align:top;margin-top:6px;"  border="0">';
			...
			if (count($userStatusComments[$key]) > 0) {
				$body .= "<div style=\"float:left; background: url('http://www.flator.se/img/meny_pil_gif.gif') no-repeat 0px 0px; padding-top:8px;margin-left:0px; margin-top:5px; \" id=\"statusComment".$key."\" >";
				while ( list( $key3, $value3 ) = each( $userStatusComments[$key] ) )
					...
					if ( count( $guestImage ) > 0 )
					{
						...
						$avatar = "<img src=\"" . $baseUrl . "/user-photos/" . str_replace('http://www.flator.se/rwdx/user/', '', $guestImage["imageUrl"]) . "/profile-small/\" style=\"margin-bottom:8px; margin-left:4px;margin-top:8px; margin-right:5px;\" />";
					...
					if ( count( $guestImage ) > 0 )
					{
						...
						$mediumAvatar = "" . $baseUrl . "/user-photos/" . str_replace('http://www.flator.se/rwdx/user/', '', $guestImage["imageUrl"]) . "/profile/";
					}
					else
					{
...
<input type=\"submit\" name=\"Skicka\" value=\"\" class=\"btnSearch\" /></form></td>";
			...
			if ($userStatus[ $key ]["statusType"] == "personalMessage") { 
					...
					$body .= "<tr><td colspan=\"3\"><div style=\" background: url('http://www.flator.se/img/meny_pil_gif.gif') no-repeat 7px 2px; padding-top:0px;display:none; \" id=\"statusComment".$key."\" ><table width=\"370px\" border=\"0\" cellpadding=\"0px\" cellspacing=\"4px\" style=\"margin-left:0px; border: 1px dotted #c8c8c8; margin-top:10px;padding:0px; background-color:#C8C8C8;\">";
					$body .= "<tr style=\" width:100%;\">";
					$body .= "<td width=\"100%\" valign=\"top\" style=\"font-size:12px; background-color:#fff;\" class=\"commentBox\">
					<form method=\"POST\"  style=\"margin: 0px; padding: 0px\">
...
<br><br></form>";
	...
	if ( count( $userGuestbook ) > 0 )
	{
		while ( list( $key, $value ) = each( $userGuestbook ) )
		{
			...
			if ( count( $guestImage ) > 0 )
			{
				$avatar = "<img src=\"" . $baseUrl . "/user-photos/" . str_replace('http://www.flator.se/rwdx/user/', '', $guestImage["imageUrl"]) . "/profile-thumb/" . "\" border=\"0\"  />";
